...
export const broadcastMessage = async (req, res) => {
    ...
    if (!subject || !subject.trim()) {
        return res.status(400).json({ error: "El asunto del comunicado es obligatorio." });
    }
        ...
        try {
            ...
            for (const user of activeUsers) {
                await connection.query(
                    ...
                    ) VALUES (?, ?, ?, NOW(), ?, 'ACEPTADA', 'Online', ?, null, '1 hora', 0)`,
                    [adminId, user.id, subjectId, message.trim(), subject.trim()]
                );
            }
